/*
Author : Mikira
Wed 11 Dec 2024 11:09:48 PM HKT
*/
...
struct BigInt {
  ...
  string toString() {
    ...
    for (int i = tmpLen - 1; i >= 0; i--) {
      string curAddition = to_string(a[i]);
      if (i != tmpLen - 1) {
        if (BASE_DIGITS - curAddition.size() > 0) {
          reverse(all(curAddition));
          rep(pad, 0, BASE_DIGITS - curAddition.size() + 1) curAddition += '0';
          reverse(all(curAddition));
        }
...
void tester() {
  rep(i,0,10000){
    int lenGen = getRange(1, 30);
    char fst = (char)(getRange(1, 9) + '0');
    string tmpGen = "";
    tmpGen += fst;
    rep(j,0,lenGen) {
      tmpGen += (char)(getRange(0, 9) + '0');
    }
    if(tmpGen != (BigInt(tmpGen)).toString()) {
      cout << tmpGen << endl;
      cout << (BigInt(tmpGen)).toString() << endl;
    }
  }
}

int main() {
  // BigInt aasb("200001998914");
  // cout << aasb.toString() << endl;
  // tester();
  // return 0;
  map <BigInt, uint64_t> stones;
  LL tmpStone;
  while (cin >> tmpStone) {
    stones[tmpStone]++;
  }
  rep(iter, 0, 100000) {
    if(iter%4000 == 0) cout << iter << endl;
    // cout << "itering " << endl;
    // cout << stones.size() << endl;
    map <BigInt, uint64_t> stoneNx;
    trav(iterStone, stones) {
      auto stone = iterStone.fi;
      string stoneStr = stone.toString();
      // cout << "Checking " << stoneStr << endl;
      int curSize = stoneStr.length();
      if (stone == 0) {
        stoneNx[1] += iterStone.se;
      } else if (stoneStr.length() % 2) {
        stoneNx[stone * 2024] += iterStone.se;
      } else {
        string tmp = "";
        rep(len, 0, curSize / 2) {
          tmp += stoneStr[len];
        ...
        // cout << tmp << endl;
        stoneNx[BigInt(tmp)] += iterStone.se;
        tmp = "";
        rep(len, curSize / 2, curSize) {
          tmp += stoneStr[len];
        }
        // cout << tmp << endl;
        stoneNx[BigInt(tmp)] += iterStone.se;
